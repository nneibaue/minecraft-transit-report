...
- [x] 02-07-PLAN.md — Harness docs, CLAUDE.md amendments, paid devices-question run post-swap (2nd of 2 paid calls) (Wave 6)
...
- [x] 03-01-PLAN.md — Deploy engine: Settings.server_dir, TOML allow-rule logic, server-running gate, marker scan, file placement, launcher (Wave 1)
- [x] 03-02-PLAN.md — Lua amendments: bridge-URL fallback/override (D-06) and device naming (D-10) in chat.lua and client.lua (Wave 1)
...
- [x] 03-04-PLAN.md — In-game installer and self-updating boot: install.lua (wget run, masked token prompt, URL default), repo startup.lua (update from main, marker skip, role launch), deploy copies it, Lua invariant tests (Wave 3)
...
- [x] 03-06-PLAN.md — README/CLAUDE.md recipe with the real captured text as the update payload, push and reboot to prove the no-PC-step update path (Wave 5)
...
- [x] 04-02-PLAN.md — Base computer alone: pre-flight, the ATM9 question with the inline fix loop, the no-second-computer reply and the first devices question (Wave 2)

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 04-03-PLAN.md — Second computer install, devices question #2, tool error, debug off, LOOP-05 byte compare, fix list and paid-call count (Wave 3)
...
| 4. In-Game Round Trip | 3/3 | In Progress|  |
